...
void sas1() {
    ...
    *   Мы можем передать l-value в функцию std::move(), и std::move() вернет нам ссылку r-value. Для работы с std::move() нужно подключить заголовочный файл utility.
    */

    const int ref1 = 12;                // константная ссылка l-value на r-value (продолжение жизни литерала)
    // ref1 = 9;                        // невозможно, ссылка константная
    ...
    Auto_ptr<int> ptr1(new int(12));    // умный указатель для работы с динамической памятью
...
void sas2() {
    ...
    *   и только потом столкнуться с исключением, из-за чего будет утечка памяти, т.к. память запросили, а адрес утерян
